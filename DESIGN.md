...
The one deliberate exception to the Inputs spec below: the contact-status `<select>` (`ContactsTable.jsx`) renders as a solid-fill, `rounded-full`, borderless pill colored by its own value (not brand-derived — see The Fixed-Severity Rule), the same register as a rubber stamp on an index card. It's a status control, not a text field, so it doesn't inherit the ruled-input treatment. All three status colors are fixed semantic colors (red/amber/green) at full opacity, and must read at equal weight. None of them is brand-derived. An accent-tinted pill (`bg-brand-accent/45`) shipped here previously and read as disabled beside its saturated red and green siblings, because a chapter's accent is not guaranteed to read as "in progress" at all; the amber matches `CONFLICT_STYLES.warning`, so one amber carries "caution" across conflict severity, budget alerts, and contact status.
